...
void AThe_Grid_CPPCharacter::Tick(float DeltaTime)
{
	...
	if (AThe_Grid_CPPCharacter::Trace(GetWorld(), this, Start, End, HitData, ECC_Pawn, false))
	{
		...
		if (HitData.GetActor())
		{
			GEngine->AddOnScreenDebugMessage(-1, 0.005f, FColor::Cyan, TEXT("---Traced---"));
			//Print out Name of Hit Actor
			GEngine->AddOnScreenDebugMessage(-1, 0.005f, FColor::Green, FString(HitData.GetActor()->GetName()));
			//Print out distance from start of trace to impact point
			GEngine->AddOnScreenDebugMessage(-1, 0.005f, FColor::Green, TEXT("Trace Distance: ") + FString::SanitizeFloat(HitData.Distance));
			...
			if (FoundTile != nullptr)
			{
				...
				if (CastedTile->Can_Place("Wall", true))
				{
					CastedTile->Build_Turret("Wall");
				}
			}
			*/
		}
...
/*
void AThe_Grid_CPPCharacter::OnFire()
{
	...
	if (ProjectileClass != NULL)
		...
		if (World != NULL)
		{
			...
			World->SpawnActor<AThe_Grid_CPPProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
		}
	}
...
void AThe_Grid_CPPCharacter::GiveDefaultWeapon()
{
	...
	AWeapon* Spawner = GetWorld()->SpawnActor<AWeapon>(WeaponSpawn);
		if (Spawner)
		{			
			//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("GiveDefaultWeapon -> Spawner"));
	...
	if (Spawner)
	{
	Spawner->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));//AttachRootComponentTo(Mesh1P, "GripPoint", EAttachLocation::SnapToTarget);
...
void AThe_Grid_CPPCharacter::FireWeapon()
{
	if (CurrentWeapon != NULL)
	{
		...
		//subtract 1 ammo from LoadedAmmo OnFire
